...
> [!WARNING]
> 本プロジェクトは現在、積極的な開発を行っていません。
> 今後の更新は、重大な不具合修正など必要最低限の対応に限られる可能性があります。
...
Project DM-D.S.SとAXISは同時にWebSocket接続が可能です。
（例: AXISの情報を先に受信し、その後にDM-D.S.Sの情報が届いた場合は、DM-D.S.Sの内容が自動的にAXISの情報に上書きされて表示されます。）
